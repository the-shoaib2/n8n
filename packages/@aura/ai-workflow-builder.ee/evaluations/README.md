...
This module provides a evaluation framework for testing the AI Workflow Builder's ability to generate correct aura workflows from natural language prompts.
...
│       ├── expressions-evaluator.ts        # aura expression syntax evaluation
...
**Expressions Evaluator**: Validates aura expression syntax and data references
...
3. **Expressions** (15% weight)
   - Are aura expressions syntactically correct?
...
This file contains all aura node type definitions and is used by the AI Workflow Builder agent to:
- Know what nodes are available in aura
- Understand node parameters and their schemas
- Generate valid workflows with proper node configurations

**Why is this required?**
The AI Workflow Builder agent needs access to node definitions to generate workflows. In a normal aura runtime, these definitions are loaded automatically. However, since the evaluation framework instantiates the agent without a running aura instance, we must provide the node definitions manually via `nodes.json`.

**How to generate nodes.json:**
1. Run your aura instance
2. Download the node definitions from locally running aura instance(http://localhost:5678/types/nodes.json)
