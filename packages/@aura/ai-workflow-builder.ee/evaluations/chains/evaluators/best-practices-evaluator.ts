...
const systemPrompt = `You are an expert aura workflow evaluator focusing specifically on BEST PRACTICES ADHERENCE.
...
aura workflows can have multiple triggers and execution paths. When evaluating whether components are "connected," understand that aura supports multiple connection methods beyond direct node-to-node data flow.
...
5. **Data Storage Sharing**: Multiple workflows reading/writing to the same persistent storage
   ...
   - Data Tables (aura's built-in storage)
...
If a best practice states that certain configuration should be applied, for example disabling aura attribution
...
const humanTemplate = `Evaluate how well this workflow follows aura best practices in the context of what the user requested.
...
async function loadRelevantBestPractices(
	...
	try {
		...
		if (relevantDocs.length === 0) {
			return {
				documentation:
					'No specific best practices documentation available for this workflow type. Evaluate based on general aura workflow principles.',
		...
		return {
			documentation:
				'Unable to load specific best practices. Evaluate based on general aura workflow principles.',
			techniques: [],
		};
	}
}
