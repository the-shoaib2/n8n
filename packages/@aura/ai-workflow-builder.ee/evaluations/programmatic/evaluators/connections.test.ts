...
describe('evaluateConnections', () => {
	const mockNodeTypes = mock<INodeTypeDescription[]>([
		{
			name: 'aura-nodes-test.manualTrigger',
		...
		{
			name: 'aura-nodes-test.code',
			displayName: 'Code',
			inputs: [NodeConnectionTypes.Main],
			outputs: [NodeConnectionTypes.Main],
		},
		{
			name: 'aura-nodes-test.httpRequest',
			displayName: 'HTTP Request',
			inputs: [NodeConnectionTypes.Main],
			outputs: [NodeConnectionTypes.Main],
		},
		{
			name: 'aura-nodes-test.openAi',
			displayName: 'OpenAI',
			inputs: `={{(() => { return [{ type: "${NodeConnectionTypes.Main}" }, { type: "${NodeConnectionTypes.AiTool}", displayName: "Tools" }]; })()}}`,
			outputs: [NodeConnectionTypes.Main],
		},
		{
			name: 'aura-nodes-test.agent',
		...
		{
			name: 'aura-nodes-test.chatOpenAi',
			displayName: 'Chat OpenAI',
			inputs: [],
			outputs: [NodeConnectionTypes.AiLanguageModel],
		},
		{
			name: 'aura-nodes-test.vectorStore',
		...
		{
			name: 'aura-nodes-test.embeddingsOpenAi',
	...
	describe('basic workflow connections validation', () => {
				...
				nodes: [
					{
						id: '1',
						name: 'Manual Trigger',
						type: 'aura-nodes-test.manualTrigger',
						parameters: {},
						typeVersion: 1,
						position: [0, 0],
					},
					...
					},
					{
						id: '3',
						name: 'HTTP Request',
						type: 'aura-nodes-test.httpRequest',
						parameters: {},
						typeVersion: 1,
						position: [400, 0],
					},
		...
		it('should detect missing node type', () => {
			...
			expect(violations).toContainEqual(
				expect.objectContaining({
					description: 'Node type aura-nodes-test.unknown not found for node Unknown Node',
				}),
			);
		});
		...
		it('should detect missing required inputs', () => {
			...
			expect(violations).toContainEqual(
				expect.objectContaining({
					description:
						'Node LLM Chain (aura-nodes-test.llmChain) is missing required input of type main',
				}),
			);
			expect(violations).toContainEqual(
				expect.objectContaining({
					description:
						'Node LLM Chain (aura-nodes-test.llmChain) is missing required input of type ai_languageModel',
				}),
			);
		});
		...
		it('should detect unsupported connection types', () => {
				...
				nodes: [
					{
						id: '1',
						name: 'Chat OpenAI',
						type: 'aura-nodes-test.chatOpenAi',
						parameters: {},
						typeVersion: 1,
						position: [0, 0],
					},
			...
			expect(violations).toContainEqual(
				expect.objectContaining({
					description:
						'Node Code Node (aura-nodes-test.code) received unsupported connection type ai_languageModel',
				}),
			);
		});
			...
			expect(violations).toContainEqual(
				expect.objectContaining({
					description: expect.stringContaining(
						'Sub-node Chat Model (aura-nodes-test.chatOpenAi) provides ai_languageModel but is not connected to a root node.',
					),
				}),
			);
		});
			...
			const workflow = mock<SimpleWorkflow>({
				nodes: [
					...
					},
					{
						...
						type: 'aura-nodes-test.vectorStore',
						typeVersion: 1.3,
						position: [0, 0],
					},
					{
					...
					},
					{
						...
						type: 'aura-nodes-test.embeddingsOpenAi',
						typeVersion: 1.2,
						position: [0, 0],
					},
				],
	...
	describe('dynamic input/output resolution', () => {
		...
		it('should resolve vector store inputs based on mode', () => {
			...
			expect(violations).toContainEqual(
				expect.objectContaining({
					description:
						'Node Vector Store (aura-nodes-test.vectorStore) is missing required input of type ai_embedding',
				}),
			);
		});
	...
	describe('dangling nodes validation', () => {
		it('should detect nodes with required main input but no connections', () => {
			...
			expect(violations).toContainEqual(
				expect.objectContaining({
					description: 'Node Code2 (aura-nodes-test.code) is missing required input of type main',
				}),
			);
		});
