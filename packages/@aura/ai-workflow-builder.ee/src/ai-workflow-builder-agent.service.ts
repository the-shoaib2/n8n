...
export class AiWorkflowBuilderService {
	...
	private filterNodeTypes(nodeTypes: INodeTypeDescription[]): INodeTypeDescription[] {
		...
		const ignoredTypes = new Set([
			'@aura/aura-nodes-langchain.toolVectorStore',
			'@aura/aura-nodes-langchain.documentGithubLoader',
			'@aura/aura-nodes-langchain.code',
		]);
		...
		const visibleNodeTypes = nodeTypes.filter(
			(nodeType) =>
				...
				(nodeType.hidden !== true || nodeType.name === 'aura-nodes-base.dataTable'),
		);
	...
	private async getAgent(user: IUser) {
		...
		const agent = new WorkflowBuilderAgent({
			...
			tracer: tracingClient
				? new LangChainTracer({ client: tracingClient, projectName: 'aura-workflow-builder' })
				: undefined,
