...
describe('AiWorkflowBuilderService', () => {
	...
	const mockNodeTypeDescriptions: INodeTypeDescription[] = [
		...
		{
			name: 'aura-nodes-base.dataTable',
		...
		{
			name: '@aura/aura-nodes-langchain.toolVectorStore',
	...
	describe('constructor', () => {
		...
		it('should filter out ignored types and hidden nodes except the data table', () => {
			...
			expect(
				filteredNodeTypes.find((node) => node.name === '@aura/aura-nodes-langchain.toolVectorStore'),
			).toBeUndefined();
			expect(
				filteredNodeTypes.find((node) => node.name === 'aura-nodes-base.dataTable'),
			).toMatchObject({ name: 'aura-nodes-base.dataTable' });
		});
	...
	describe('chat', () => {
		...
		it('should create WorkflowBuilderAgent with correct configuration when using client', async () => {
			...
			expect(config).toHaveProperty('instanceUrl', 'https://aura.example.com');
