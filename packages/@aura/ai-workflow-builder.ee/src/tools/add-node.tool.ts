...
import type { INode, INodeParameters, INodeTypeDescription } from 'aura-workflow';
import { z } from 'zod';
...
const baseSchema = {
	nodeType: z.string().describe('The type of node to add (e.g., aura-nodes-base.httpRequest)'),
...
- Vector Store (@aura/aura-nodes-langchain.vectorStoreInMemory):
  - For document input: { mode: "insert" }
  - For querying: { mode: "retrieve" }
  - For AI tool use: { mode: "retrieve-as-tool" }
- Document Loader (@aura/aura-nodes-langchain.documentDefaultDataLoader):
