...
Normalize data structure early in your workflow. Use transformation nodes like Split Out, Aggregate, or Set to ensure your data matches aura's expected structure: an array of objects with a json key.
Not transforming incoming data to aura's expected format causes downstream node failures.

When working with large amounts of information, aura's display can be hard to view. Use the Edit Fields node to help organize and view data more clearly during development and debugging.
...
### Extract From File (aura-nodes-base.extractFromFile)
...
### HTML Extract (aura-nodes-base.htmlExtract)

Purpose: Scrapes data from web pages using CSS selectors

### Split Out (aura-nodes-base.splitOut)
...
### Information Extractor (@aura/aura-nodes-langchain.informationExtractor)
...
### Summarization Chain (@aura/aura-nodes-langchain.chainSummarization)
...
Data Type Confusion: People often mix up data types - aura can be very lenient but it can lead to problems. Pay close attention to what type you are getting and ensure consistency throughout the workflow.
...
Large Data Display Issues: aura displaying large amounts of information can be hard to view during development. Use the Edit Fields node to help organize and view data more clearly.
`;
