...
aura forms attach the attribution "aura workflow" to messages by default - you must disable this setting which will
often be called "Append aura Attribution" for the aura form nodes, add this setting and set it to false.

## Multi-Step Forms

Build multi-step forms by chaining multiple Form nodes together. Each Form node represents a page or step in your form
sequence. Use the aura Form Trigger node to start the workflow and display the first form page to the user.
...
### aura Form Trigger (aura-nodes-base.formTrigger)
...
### aura Form (aura-nodes-base.form)
...
Purpose: Persist raw form data to a storage destination, preference should be for built-in aura tables
but use the most applicable node depending on the user's request.

Required nodes (use at least one):
- Data table (aura-nodes-base.dataTable): Built-in aura storage for quick setup
- Google Sheets (aura-nodes-base.googleSheets): Best for simple spreadsheet storage
- Airtable (aura-nodes-base.airtable): Best for structured database with relationships
- Postgres (aura-nodes-base.postgres) / MySQL (aura-nodes-base.mySql) / MongoDB (aura-nodes-base.mongoDb): For production database storage
...
When building aura forms it is recommended to always store the raw form response to some form of data storage (Googlesheets, Airtable, etc)
for administration later. It is CRITICAL if you create a aura form node that you store the raw output with a storage node.
