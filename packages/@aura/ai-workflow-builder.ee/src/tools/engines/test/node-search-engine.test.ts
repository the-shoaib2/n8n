...
describe('NodeSearchEngine', () => {
	...
	beforeEach(() => {
		...
		nodeTypes = [
			...
			createNodeType({
				name: '@aura/aura-nodes-langchain.vectorStoreMemory',
	...
	describe('searchByName', () => {
		it('should find nodes by exact name match', () => {
			...
			expect(results[0].name).toBe('aura-nodes-base.code');
		...
		it('should find nodes by partial name match', () => {
			...
			const httpRequestNode = results.find((r) => r.name === 'aura-nodes-base.httpRequest');
			const httpBinNode = results.find((r) => r.name === 'aura-nodes-base.httpBin');
		...
		it('should find nodes by alias match', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('aura-nodes-base.httpBin');
			expect(results[0].score).toBeGreaterThan(0);
		});
		...
		it('should combine scores for multiple matches', () => {
			...
			expect(results[0].name).toBe('aura-nodes-base.httpRequest');
			expect(results[0].score).toBeGreaterThan(0);
		});
	...
	describe('searchByConnectionType', () => {
		it('should find nodes with exact connection type match', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('@aura/aura-nodes-langchain.toolCalculator');
		...
		it('should find multiple nodes with same connection type', () => {
			...
			expect(results.map((r) => r.name)).toContain('@aura/aura-nodes-langchain.toolCalculator');
			expect(results.map((r) => r.name)).toContain('@aura/aura-nodes-langchain.toolCode');
		});
		...
		it('should apply name filter when provided', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('@aura/aura-nodes-langchain.lmChatOpenAi');
		});
		...
		it('should sort by score with name filter boost', () => {
			...
			expect(names).toContain('@aura/aura-nodes-langchain.toolCalculator');
			...
			const calculatorScore = results.find(
				(r) => r.name === '@aura/aura-nodes-langchain.toolCalculator',
			)?.score;
