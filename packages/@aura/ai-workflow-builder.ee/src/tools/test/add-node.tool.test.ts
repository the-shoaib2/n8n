...
import type { INodeTypeDescription, INode } from 'aura-workflow';
...
describe('AddNodeTool', () => {
	...
	describe('invoke', () => {
		...
		it('should handle connection parameters for AI nodes', async () => {
			...
			const result = await addNodeTool.invoke(
				buildAddNodeInput({
					nodeType: '@aura/aura-nodes-langchain.agent',
					name: 'AI Assistant',
		...
		it('should handle unknown node type', async () => {
			...
			const result = await addNodeTool.invoke(
				buildAddNodeInput({
					nodeType: 'aura-nodes-base.unknown',
					name: 'Unknown Node',
			...
			const content = parseToolResult<ParsedToolContent>(result);
			expectToolError(content, 'Error: Node type "aura-nodes-base.unknown" not found');
		});
		...
		it('should handle webhook nodes with special properties', async () => {
			...
			const result = await addNodeTool.invoke(
				buildAddNodeInput({
					nodeType: 'aura-nodes-base.webhook',
			...
			expect(addedNode?.type).toBe('aura-nodes-base.webhook');
		...
		it('should fail when requesting a non-existent node version', async () => {
			...
			const content = parseToolResult<ParsedToolContent>(result);
			expectToolError(content, 'Error: Node type "aura-nodes-base.code" not found');
		});
		...
		it('should add correct version from array version node type', async () => {
			...
			const multiVersionNode = {
				...nodeTypes.code,
				name: 'aura-nodes-base.multiVersionCode',
			...
			const result = await toolWithArrayVersion.invoke(
				buildAddNodeInput({
					nodeType: 'aura-nodes-base.multiVersionCode',
					nodeVersion: 2,
			...
			expectNodeAdded(content, {
				name: 'Multi Version Code V2',
				type: 'aura-nodes-base.multiVersionCode',
			});
		...
		it('should fail when requesting version not in array version node type', async () => {
			...
			const content = parseToolResult<ParsedToolContent>(result);
			expectToolError(content, 'Error: Node type "aura-nodes-base.multiVersionCode" not found');
		});
	});
