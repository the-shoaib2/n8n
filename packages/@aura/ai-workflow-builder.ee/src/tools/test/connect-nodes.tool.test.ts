...
import type { IConnections, INodeTypeDescription } from 'aura-workflow';
...
describe('ConnectNodesTool', () => {
	...
	describe('invoke', () => {
		...
		it('should auto-swap nodes when AI sub-node is specified as target', async () => {
			...
			const existingWorkflow = createWorkflow([
				createNode({ id: 'agent1', name: 'AI Agent', type: '@aura/aura-nodes-langchain.agent' }),
				createNode({
					id: 'tool1',
					name: 'Calculator',
		...
		it('should connect AI sub-nodes with proper connection type', async () => {
			...
			nodeTypesList = nodeTypesList.filter((nt) => nt.name !== '@aura/aura-nodes-langchain.agent');
		...
		it('should handle custom source and target indices', async () => {
			const existingWorkflow = createWorkflow([
				createNode({ id: 'node1', name: 'Multi Output', type: 'aura-nodes-base.code' }),
				createNode({ id: 'node2', name: 'Multi Input', type: 'aura-nodes-base.code' }),
			]);
			setupWorkflowState(mockGetCurrentTaskInput, existingWorkflow);
		...
		it('should handle invalid connection between incompatible nodes', async () => {
			const existingWorkflow = createWorkflow([
				createNode({ id: 'webhook1', name: 'Webhook 1', type: 'aura-nodes-base.webhook' }),
				createNode({ id: 'webhook2', name: 'Webhook 2', type: 'aura-nodes-base.webhook' }),
			]);
			setupWorkflowState(mockGetCurrentTaskInput, existingWorkflow);
		...
		it('should handle multiple output types and pick the right one', async () => {
			...
			const existingWorkflow = createWorkflow([
				...
				createNode({ id: 'code1', name: 'Code', type: 'aura-nodes-base.code' }),
			]);
			setupWorkflowState(mockGetCurrentTaskInput, existingWorkflow);
