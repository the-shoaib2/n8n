	...
	describe('invoke', () => {
		it('should remove a node without connections', async () => {
			const existingWorkflow = createWorkflow([
				createNode({ id: 'node1', name: 'Code', type: 'aura-nodes-base.code' }),
				createNode({ id: 'node2', name: 'HTTP Request', type: 'aura-nodes-base.httpRequest' }),
			]);
			...
			expectToolSuccess(content, 'Successfully removed node "Code" (aura-nodes-base.code)');
			...
			expect(completeMessage?.updates[0]?.data).toMatchObject({
				...
				removedNodeType: 'aura-nodes-base.code',
		...
		it('should handle removing node with AI connections', async () => {
			const existingWorkflow = createWorkflow([
				...
				createNode({ id: 'node2', name: 'AI Agent', type: '@aura/aura-nodes-langchain.agent' }),
				createNode({
					id: 'node3',
		...
		it('should handle removing node by exact ID match', async () => {
			const existingWorkflow = createWorkflow([
				createNode({ id: 'test-uuid-123', name: 'My Node', type: 'aura-nodes-base.set' }),
			]);
			setupWorkflowState(mockGetCurrentTaskInput, existingWorkflow);
			...
			expectToolSuccess(content, 'Successfully removed node "My Node" (aura-nodes-base.set)');
		...
		it('should handle different connection types', async () => {
			const existingWorkflow = createWorkflow([
				createNode({
					id: 'node1',
					name: 'Vector Store',
					type: '@aura/aura-nodes-langchain.vectorStore',
				}),
				createNode({
					id: 'node2',
					name: 'Embeddings',
				...
				createNode({
					...
					type: '@aura/aura-nodes-langchain.documentLoader',
				}),
			]);
		...
		it('should return correct output structure', async () => {
			...
			expect(completeMessage?.updates[0]?.data).toEqual({
				...
				removedNodeType: 'aura-nodes-base.httpRequest',
				connectionsRemoved: 0,
				message: 'Successfully removed node "Test Node" (aura-nodes-base.httpRequest)',
			});
		});
	});
});
