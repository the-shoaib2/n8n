...
describe('UpdateNodeParametersTool', () => {
	...
	describe('invoke', () => {
		it('should update node parameters successfully', async () => {
			...
			expectToolSuccess(
				content,
				'Successfully updated parameters for node "HTTP Request" (aura-nodes-base.httpRequest):',
			);
		...
		it('should pass correct context to parameter updater chain', async () => {
			...
			expect(mockChain.invoke).toHaveBeenCalledWith(
				expect.objectContaining({
					...
					node_type: 'aura-nodes-base.httpRequest',
		...
		it('should handle array version node types correctly', async () => {
			...
			expectToolSuccess(
				content,
				'Successfully updated parameters for node "Multi Version Code" (aura-nodes-base.code):',
			);
