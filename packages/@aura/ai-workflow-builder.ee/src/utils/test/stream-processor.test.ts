			...
			it('should process tool progress chunks', () => {
				const toolChunk: ToolProgressChunk = {
					...
					updates: [
						{
							type: 'input',
							data: { nodeType: 'aura-nodes-base.code' },
						},
					],
				};
...
[{"nodeName": "Manual Trigger"}]
			...
			aiMessage.tool_calls = [
				{
					id: 'call-1',
					name: 'add_nodes',
					args: { nodeType: 'aura-nodes-base.code' },
					type: 'tool_call',
				},
				{
					id: 'call-2',
		...
		it('should handle ToolMessage with object content', () => {
			...
			const toolMessage = new ToolMessage({
				...
				content: { nodeId: 'node1', nodeType: 'aura-nodes-base.code' },
				tool_call_id: 'call-1',
			});
			...
			expect(result[0].updates?.[1]).toEqual({
				type: 'output',
				data: { nodeId: 'node1', nodeType: 'aura-nodes-base.code' },
			});
		});
