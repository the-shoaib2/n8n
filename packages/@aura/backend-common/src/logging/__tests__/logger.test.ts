...
import type { GlobalConfig, InstanceSettingsConfig } from '@aura/config';
		...
		describe('`file`', () => {
			test('should set file transport', () => {
				...
				const logger = new Logger(
					globalConfig,
					mock<InstanceSettingsConfig>({ auraFolder: '/tmp' }),
				);
			...
			test('should accept absolute paths', () => {
				...
				// ACT
				new Logger(globalConfig, mock<InstanceSettingsConfig>({ auraFolder: '/tmp' }));
			...
			test('should accept relative paths', () => {
				...
				// ACT
				new Logger(globalConfig, mock<InstanceSettingsConfig>({ auraFolder }));
				...
				expect(FileSpy).toHaveBeenCalledTimes(1);
				expect(FileSpy).toHaveBeenCalledWith(fileOptionsCaptor);
				expect(fileOptionsCaptor.value).toMatchObject({ filename: `${auraFolder}/${location}` });
			});
		});
	});
