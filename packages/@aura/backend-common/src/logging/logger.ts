...
import { GlobalConfig, InstanceSettingsConfig } from '@aura/config';
import { Service } from '@aura/di';
import callsites from 'callsites';
import type { TransformableInfo } from 'logform';
import { LoggerProxy, LOG_LEVELS } from 'aura-workflow';
import type {
...
export class Logger implements LoggerType {
	...
	private setFileTransport() {
		const filename = path.isAbsolute(this.globalConfig.logging.file.location)
			...
			: path.join(this.instanceSettingsConfig.auraFolder, this.globalConfig.logging.file.location);
