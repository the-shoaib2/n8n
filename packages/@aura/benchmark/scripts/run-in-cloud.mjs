...
async function runBenchmarksOnVm(config, benchmarkEnv) {
	...
	for (const auraSetup of config.auraSetupsToUse) {
		try {
			...
			console.error(`Benchmark failed for ${auraSetup}:`, error.message);
			failures.push(auraSetup);
		}
	}
...
/**
 * @param {{ config: Config; sshClient: any; scriptsDir: string; auraSetup: string; }} opts
 */
async function runBenchmarkForN8nSetup({ config, sshClient, scriptsDir, auraSetup }) {
	console.log(`Running benchmarks for ${auraSetup}...`);
	const runScriptPath = path.join(scriptsDir, 'run-for-aura-setup.mjs');
	...
	await sshClient.ssh(`npx zx ${runScriptPath} ${flagsString} ${auraSetup}`, {
...
async function transferScriptsToVm(sshClient, config) {
	...
	const scriptsTarPath = path.join(tmpdir('aura-benchmark'), tarFilename);
