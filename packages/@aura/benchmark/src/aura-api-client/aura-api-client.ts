...
export class N8nApiClient {
	...
	async waitForInstanceToBecomeOnline(): Promise<void> {
		...
		while (Date.now() - START_TIME < TIMEOUT_MS) {
			...
			console.log(`aura instance not online yet, retrying in ${INTERVAL_MS / 1000} seconds...`);
			await this.delay(INTERVAL_MS);
		}

		throw new Error(`aura instance did not come online within ${TIMEOUT_MS / 1000} seconds`);
	}
	...
	async setupOwnerIfNeeded(loginDetails: { email: string; password: string }) {
		...
		if (response.status === 200) {
			...
			// The aura instance setup owner endpoint not be available yet even tho
