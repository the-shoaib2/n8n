...
export class GlobalConfig {
	...
	/** Path aura is deployed to */
	@Env('N8N_PATH')
	path: string = '/';

	/** Host name aura can be reached */
	@Env('N8N_HOST')
	host: string = 'localhost';

	/** HTTP port aura can be reached */
	@Env('N8N_PORT')
	port: number = 5678;

	/** IP address aura should listen on */
	@Env('N8N_LISTEN_ADDRESS')
	listen_address: string = '::';

	/** HTTP Protocol via which aura can be reached */
	...
	/** Number of reverse proxies aura is running behind. */
	@Env('N8N_PROXY_HOPS')
	proxy_hops: number = 0;
	...
	/** Public URL where the editor is accessible. Also used for emails sent from aura. */
