...
describe('GlobalConfig', () => {
	...
	const defaultConfig: GlobalConfig = {
		...
		nodes: {
			errorTriggerType: 'aura-nodes-base.errorTrigger',
			include: [],
			exclude: [],
			pythonEnabled: true,
		},
		...
		license: {
			serverUrl: 'https://license.aura.io/v1',
		...
		diagnostics: {
			enabled: true,
			frontendConfig: '1zPn9bgWPzlQc0p8Gj1uiK6DOTn;https://telemetry.aura.io',
			backendConfig: '1zPn7YoGC3ZXE9zLeTKLuQCB4F6;https://telemetry.aura.io',
			posthogConfig: {
		...
		sso: {
			...
			provisioning: {
				...
				scopesName: 'aura',
				scopesInstanceRoleClaimName: 'aura_instance_role',
				scopesProjectsRolesClaimName: 'aura_projects',
			},
		},
		redis: {
			prefix: 'aura',
	...
	it('should use values from env variables when defined', () => {
		...
		expect(structuredClone(config)).toEqual({
			...
			nodes: {
				...defaultConfig.nodes,
				include: ['aura-nodes-base.hackerNews'],
			},
			templates: {
