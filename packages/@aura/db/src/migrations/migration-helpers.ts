...
function loadSurveyFromDisk(): string | null {
	try {
		const filename = `${
			Container.get(InstanceSettings).auraFolder
...
export const wrapMigration = (migration: Migration) => {
	const prototype = migration.prototype as unknown as { __aura_wrapped?: boolean };
	if (prototype.__aura_wrapped === true) {
		return;
	}
	prototype.__aura_wrapped = true;
