...
export const NoUncaughtJsonParseRule = ESLintUtils.RuleCreator.withoutDocs({
	meta: {
		...
		docs: {
			description:
				'Calls to `JSON.parse()` must be replaced with `jsonParse()` from `aura-workflow` or surrounded with a try/catch block.',
		},
		schema: [],
		messages: {
			noUncaughtJsonParse:
				'Use `jsonParse()` from `aura-workflow` or surround the `JSON.parse()` call with a try/catch block.',
		},
	},
	defaultOptions: [],
