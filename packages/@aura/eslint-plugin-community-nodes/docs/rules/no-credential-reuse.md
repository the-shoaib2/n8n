# Prevent credential re-use security issues by ensuring nodes only reference credentials from the same package (`@aura/community-nodes/no-credential-reuse`)
...
// package.json: "aura": { "credentials": ["dist/credentials/MyApiCredential.credentials.js"] }
