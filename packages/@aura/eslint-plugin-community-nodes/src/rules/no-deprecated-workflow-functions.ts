...
export const NoDeprecatedWorkflowFunctionsRule = createRule({
	...
	meta: {
		type: 'problem',
		docs: {
			description: 'Disallow usage of deprecated functions and types from aura-workflow package',
		},
		messages: {
	...
	defaultOptions: [],
	create(context) {
		const auraWorkflowTypes = new Set<string>();

		return {
			ImportDeclaration(node) {
				if (node.source.value === 'aura-workflow') {
					node.specifiers.forEach((specifier) => {
						if (
							specifier.type === AST_NODE_TYPES.ImportSpecifier &&
							specifier.imported.type === AST_NODE_TYPES.Identifier
						) {
							auraWorkflowTypes.add(specifier.local.name);
						}
					});
				}
			},
			...
			TSTypeReference(node) {
				if (
					...
					auraWorkflowTypes.has(node.typeName.name)
				) {
			...
			ImportSpecifier(node) {
				// Check if this import is from aura-workflow by looking at the parent ImportDeclaration
				const importDeclaration = node.parent;
				if (
					importDeclaration?.type === AST_NODE_TYPES.ImportDeclaration &&
					importDeclaration.source.value === 'aura-workflow' &&
