...
ruleTester.run('no-restricted-imports', NoRestrictedImportsRule, {
	valid: [
		{
			code: 'import { WorkflowExecuteMode } from "aura-workflow";',
		},
		{
		...
		{
			code: 'const workflow = await import("aura-workflow");',
		},
		{
		...
		{
			code: 'require.resolve(`aura-workflow`);',
		},
		{
			code: 'const workflow = await import(`aura-workflow`);',
		},
...
import { WorkflowExecuteMode } from "aura-workflow";`,
			errors: [
...
const workflow = await import("aura-workflow");`,
			errors: [
