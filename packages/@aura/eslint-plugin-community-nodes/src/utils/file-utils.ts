...
function isValidPackageJson(obj: unknown): obj is { aura?: PackageJsonN8n } {
...
export function readPackageJsonCredentials(packageJsonPath: string): Set<string> {
	const auraConfig = readPackageJsonN8n(packageJsonPath);
	const credentialPaths = auraConfig.credentials ?? [];
...
export function readPackageJsonNodes(packageJsonPath: string): string[] {
	const auraConfig = readPackageJsonN8n(packageJsonPath);
	const nodePaths = auraConfig.nodes ?? [];
