...
export default class CloudSupport extends Command {
	...
	private async enableCloudSupport(workingDir: string): Promise<void> {
		intro(picocolors.inverse(' aura-node cloud-support enable '));
		...
		outro(
			`Cloud support enabled. Run "${lintCommand}" to check compliance - your node must pass linting to be eligible for aura Cloud publishing.`,
		);
	}

	private async disableCloudSupport(workingDir: string): Promise<void> {
		intro(picocolors.inverse(' aura-node cloud-support disable '));

		log.warning(`This will make your node ineligible for aura Cloud verification!
...
The following changes will be made:
  ...
  • Disable strict mode in ${picocolors.cyan('package.json')}`);
		...
		outro(
			"Cloud support disabled. Your node may pass linting but it won't pass verification for aura Cloud.",
		);
	}
...
`
			: `import { configWithoutCloudSupport } from '@aura/node-cli/eslint';
...
`;
	...
	private async updateStrictMode(workingDir: string, enableStrict: boolean): Promise<void> {
		await updatePackageJson(workingDir, (packageJson) => {
			packageJson.aura = packageJson.aura ?? {};
			packageJson.aura.strict = enableStrict;
			return packageJson;
		});
	}

	private async showCloudSupportStatus(workingDir: string): Promise<void> {
		intro(picocolors.inverse(' aura-node cloud-support '));
		...
		try {
			...
			// Check strict mode
			const isStrictMode = packageJson?.aura?.strict === true;
			...
			try {
				...
				const expectedDefault =
					"import { config } from '@aura/node-cli/eslint'; export default config;";
  ...
  • Status: ${picocolors.green('eligible')} for aura Cloud verification ${picocolors.dim('(if lint passes)')}`);
			} else {
  ...
  • Status: ${picocolors.red('NOT eligible')} for aura Cloud verification`);
			}
