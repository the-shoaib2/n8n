...
`;
	...
	tmpdirTest('invalid package - not an aura node package', async ({ tmpdir }) => {
		await fs.writeFile(
			`${tmpdir}/package.json`,
			JSON.stringify({
				name: 'regular-package',
				version: '1.0.0',
				// No aura field - this makes it an invalid aura package
			}),
		);

		await expect(CommandTester.run('lint')).rejects.toThrow('EEXIT: 1');

		expect(cancel).toHaveBeenCalledWith('lint can only be run in an aura node package');
	});

	tmpdirTest('strict mode with default config - passes validation', async ({ tmpdir }) => {
		await setupTestPackage(tmpdir, {
			packageJson: { aura: { strict: true } },
			eslintConfig: true,
		});
	...
	tmpdirTest('cloud-only lint errors - suggests disabling cloud support', async ({ tmpdir }) => {
		...
		mockSpawn('pnpm', ['exec', '--', 'eslint', '.'], {
			exitCode: 1,
			stderr: 'Error: @aura/eslint-plugin-community-nodes/no-restricted-globals rule failed',
		});
		...
		const hasCloudMessage = stdoutCalls.some(
			(call) =>
				typeof call === 'string' && call.includes('aura Cloud compatibility issues detected'),
		);
	...
	tmpdirTest('regular lint errors - no cloud suggestion', async ({ tmpdir }) => {
		...
		const stdoutCalls = mockProcessStdout.mock.calls.flat();
		const hasCloudMessage = stdoutCalls.some(
			(call) => typeof call === 'string' && call.includes('aura Cloud compatibility'),
		);
