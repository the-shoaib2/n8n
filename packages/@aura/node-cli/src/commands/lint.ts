...
To disable strict mode: set ${picocolors.yellow('"strict": false')} in ${picocolors.cyan('package.json')} under the ${picocolors.yellow('"aura"')} section.`);
				process.exit(1);
			}
		} catch (error: unknown) {
...
To create default config: ${enableCommand}`,
	...
	private async handleLintErrors(eslintOutput: string): Promise<void> {
		if (this.containsCloudOnlyErrors(eslintOutput)) {
			...
			this.log(`${picocolors.yellow('⚠️  aura Cloud compatibility issues detected')}

These lint failures prevent verification to aura Cloud.
...
${picocolors.dim(`Note: This will switch to ${picocolors.magenta('configWithoutCloudSupport')} and disable strict mode`)}`);
	...
	private containsCloudOnlyErrors(errorMessage: string): boolean {
		const cloudOnlyRules = [
			'@aura/eslint-plugin-community-nodes/no-restricted-globals',
			'@aura/eslint-plugin-community-nodes/no-restricted-imports',
		];
