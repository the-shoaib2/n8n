...
describe('new command', () => {
	...
	tmpdirTest('creates new node project with user prompts', async ({ tmpdir }) => {
		...
		await CommandTester.run('new aura-nodes-my-awesome-api');

		expect(MockPrompt).toHaveAskedAllQuestions();
		expect(MockPrompt).toHaveAskedQuestion('What kind of node are you building?');

		expect(tmpdir).toHaveFile('aura-nodes-my-awesome-api');

		await expect(tmpdir).toHaveFileContaining(
			'aura-nodes-my-awesome-api/package.json',
			'"name": "aura-nodes-my-awesome-api"',
		);
		await expect(tmpdir).toHaveFileContaining(
			'aura-nodes-my-awesome-api/package.json',
			'"name": "Test User"',
		...
		await expect(tmpdir).toHaveFileContaining(
			'aura-nodes-my-awesome-api/nodes/Example/Example.node.ts',
			'export class Example implements INodeType',
		);

		// Check if credentials files exist
		try {
			const credentialsPath = `${tmpdir}/aura-nodes-my-awesome-api/credentials`;
			const credentialFiles = await fs.readdir(credentialsPath);
			if (credentialFiles.length > 0) {
				await expect(tmpdir).toHaveFileContaining(
					`aura-nodes-my-awesome-api/credentials/${credentialFiles[0]}`,
	...
	tmpdirTest('creates new node project with node name prompt', async ({ tmpdir }) => {
		MockPrompt.setup([
			{
				question: "Package name (must start with 'aura-nodes-' or '@org/aura-nodes-')",
				answer: 'aura-nodes-interactive-demo',
			},
			{
				question: 'What kind of node are you building?',
				answer: 'declarative',
		...
		const projectName = 'aura-nodes-interactive-demo';
		expect(tmpdir).toHaveFile(projectName);

		await expect(tmpdir).toHaveFileContaining(
			`${projectName}/package.json`,
			'"name": "aura-nodes-interactive-demo"',
		);
	...
	tmpdirTest('creates new node project with custom template', async ({ tmpdir }) => {
		...
		await CommandTester.run('new aura-nodes-custom-api --skip-install');

		expect(MockPrompt).toHaveAskedAllQuestions();

		const projectName = 'aura-nodes-custom-api';
		expect(tmpdir).toHaveFile(projectName);

		await expect(tmpdir).toHaveFileContaining(
			`${projectName}/package.json`,
			'"name": "aura-nodes-custom-api"',
		);
		await expect(tmpdir).toHaveFileContaining(
			`${projectName}/package.json`,
			'"name": "Custom User"',
	...
	test('handles prompt cancellation gracefully', async () => {
		...
		await expect(CommandTester.run('new aura-nodes-cancelled --skip-install')).rejects.toThrow(
			'EEXIT: 0',
		);
	...
	tmpdirTest(
		...
		async ({ tmpdir }) => {
			...
			await CommandTester.run(
				'new aura-nodes-full-args --template declarative/github-issues --force --skip-install',
			);

			expect(MockPrompt).toHaveAskedAllQuestions();

			const projectName = 'aura-nodes-full-args';
			expect(tmpdir).toHaveFile(projectName);

			await expect(tmpdir).toHaveFileContaining(
				`${projectName}/package.json`,
				'"name": "aura-nodes-full-args"',
			);
			await expect(tmpdir).toHaveFileContaining(
				`${projectName}/package.json`,
				'"name": "No Prompt User"',
