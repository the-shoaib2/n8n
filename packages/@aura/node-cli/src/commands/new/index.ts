...
export default class New extends Command {
	...
	static override examples = [
		...
		'<%= config.bin %> <%= command.id %> aura-nodes-my-app --skip-install',
		'<%= config.bin %> <%= command.id %> aura-nodes-my-app --force',
		'<%= config.bin %> <%= command.id %> aura-nodes-my-app --template declarative/custom',
	];
...
📚 Documentation: https://docs.aura.io/integrations/creating-nodes/build/${type}-style-node/
