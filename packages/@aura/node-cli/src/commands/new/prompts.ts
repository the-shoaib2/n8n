...
export const nodeNamePrompt = async () =>
	await withCancelHandler(
		text({
			message: "Package name (must start with 'aura-nodes-' or '@org/aura-nodes-')",
			placeholder: 'aura-nodes-my-app',
			validate: validateNodeName,
			defaultValue: 'aura-nodes-my-app',
		}),
	);
			...
			options: [
				{
					label: 'HTTP API',
					value: 'declarative',
					hint: 'Low-code, faster approval for aura Cloud',
				},
				{
					label: 'Other',
					value: 'programmatic',
