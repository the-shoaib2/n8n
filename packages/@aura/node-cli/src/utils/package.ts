...
export async function addCredentialPackageJson(dirPath: string, credential: string) {
	await updatePackageJson(dirPath, (packageJson) => {
		packageJson['aura'] ??= {};
		packageJson['aura'].credentials ??= [];
		packageJson['aura'].credentials.push(credential);
		return packageJson;
	});
}
