...
export async function ensureN8nPackage(commandName: string) {
	...
	if (!isN8nNode) {
		log.error(`Make sure you are in the root directory of your node package and your package.json contains the "aura" field

For example:
{
	"name": "aura-nodes-my-app",
	"version": "0.1.0",
	"aura": {
...
`);
		onCancel(`${commandName} can only be run in an aura node package`, 1);
		process.exit(1);
	}
}
