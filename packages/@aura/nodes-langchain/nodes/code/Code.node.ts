...
import { NodeOperationError, NodeConnectionTypes } from 'aura-workflow';
import type {
	IExecuteFunctions,
	INodeExecutionData,
	INodeType,
...
export const vmResolver = makeResolverFromLegacyOptions({
	...
	resolve(moduleName, parentDirname) {
		if (moduleName.match(/^langchain\//) ?? moduleName.match(/^@langchain\//)) {
			return require.resolve(`@aura/aura-nodes-langchain/node_modules/${moduleName}.cjs`, {
				paths: [parentDirname],
			});
		}
		return;
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/root-nodes/aura-nodes-langchain.code/',
					},
				],
			},
		},
