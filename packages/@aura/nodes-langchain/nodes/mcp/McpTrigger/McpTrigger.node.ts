import { WebhookAuthorizationError } from 'aura-nodes-base/dist/nodes/Webhook/error';
import { validateWebhookAuthentication } from 'aura-nodes-base/dist/nodes/Webhook/utils';
import type { INodeTypeDescription, IWebhookFunctions, IWebhookResponseData } from 'aura-workflow';
import { NodeConnectionTypes, Node, nodeNameToToolName } from 'aura-workflow';
...
export class McpTrigger extends Node {
	description: INodeTypeDescription = {
		...
		group: ['trigger'],
		version: [1, 1.1, 2],
		description: 'Expose aura tools as an MCP Server endpoint',
		activationMessage:
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/core-nodes/aura-nodes-langchain.mcptrigger/',
					},
				],
			},
		},
	...
	async webhook(context: IWebhookFunctions): Promise<IWebhookResponseData> {
		...
		const serverName = node.typeVersion > 1 ? nodeNameToToolName(node) : 'aura-mcp-server';
