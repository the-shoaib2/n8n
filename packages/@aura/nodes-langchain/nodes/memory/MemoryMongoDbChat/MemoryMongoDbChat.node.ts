...
export class MemoryMongoDbChat implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/sub-nodes/aura-nodes-langchain.memorymongochat/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		if (credentials.configurationType === 'connectionString') {
			...
			connectionString = `mongodb://${authString}${host}:${port}/?appname=aura`;
			if (tls) {
