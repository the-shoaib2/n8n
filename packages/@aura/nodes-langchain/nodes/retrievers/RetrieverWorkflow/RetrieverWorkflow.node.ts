...
export class RetrieverWorkflow implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ['transform'],
		version: [1, 1.1],
		description: 'Use an aura Workflow as Retriever',
		defaults: {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/sub-nodes/aura-nodes-langchain.retrieverworkflow/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		class WorkflowRetriever extends BaseRetriever {
			lc_namespace = ['aura-nodes-langchain', 'retrievers', 'workflow'];
