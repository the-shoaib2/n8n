...
export class ToolHttpRequest implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/sub-nodes/aura-nodes-langchain.toolhttprequest/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		const requestOptions: IHttpRequestOptions = {
			...
			qs: {},
			headers: {
				// FIXME: This is a workaround to prevent the node from sending a default User-Agent (`aura`) when the header is not set.
