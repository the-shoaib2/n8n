...
import type { ICredentialDataDecryptedObject, IWebhookFunctions } from 'aura-workflow';
...
export async function validateAuth(context: IWebhookFunctions) {
	...
	if (authentication === 'none') {
	...
	} else if (authentication === 'auraUserAuth') {
		...
		const authCookie = getCookie('aura-auth');
