# @aura/stylelint-config

Stylelint configuration for aura projects with custom CSS variable naming convention enforcement.
...
This package is already configured in aura frontend packages. To use it in a new package:

```json
{
  "devDependencies": {
    "@aura/stylelint-config": "workspace:*"
  }
}
```
...
The `@aura/css-var-naming` rule enforces a structured naming pattern for CSS custom properties (variables).
...
- `namespace`: `aura`, `chat`, or `p` (for primitives)
...
/* Complex patterns */
--aura--button--background--primary--solid--hover--dark: #0a58ca;
```
...
```scss
/* In @aura/design-system */
:root {
  --aura--color--primary: #ff6d5a;
  --aura--button--background--primary: var(--aura--color--primary);
}
...
```bash
cd packages/@aura/stylelint-config
pnpm test
```
...
- **Solution**: Use `--fix` to auto-fix pattern issues, or add `/* stylelint-disable @aura/css-var-naming */` temporarily
...
```css
/* stylelint-disable @aura/css-var-naming */
```
...
- [aura Design System](../frontend/@aura/design-system/)
