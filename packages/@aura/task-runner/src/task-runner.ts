import { isSerializedBuffer, toBuffer } from 'aura-core';
import { ApplicationError, ensureError, randomInt } from 'aura-workflow';
...
export abstract class TaskRunner extends EventEmitter {
	...
	constructor(opts: TaskRunnerOpts) {
		...
		this.ws.addEventListener('error', (event) => {
			...
			if (
				...
				console.error(
					`Error: Failed to connect to aura task broker. Please ensure aura task broker is reachable at: ${taskBrokerHost}`,
				);
				process.exit(1);
			} else {
				console.error(`Error: Failed to connect to aura task broker at ${taskBrokerHost}`);
	...
	private receiveMessage = (message: MessageEvent) => {
		// eslint-disable-next-line aura-local-rules/no-uncaught-json-parse
