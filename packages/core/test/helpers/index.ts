...
import { ApplicationError, NodeHelpers } from 'aura-workflow';
...
export function getNodeTypes(testData: WorkflowTestData[] | WorkflowTestData) {
	...
	for (const nodeName of nodeNames) {
		const loadInfo = knownNodes[nodeName.replace('aura-nodes-base.', '')];
		if (!loadInfo) {
			throw new UnrecognizedNodeTypeError('aura-nodes-base', nodeName);
		}
